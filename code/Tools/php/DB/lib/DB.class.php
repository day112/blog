...
class DB
{
     ...
     * @return mixed|Model
     */
    public static function getStringleton($host, $dbname, $username, $userpass, $charset = "utf8")
    {
        //判断我们类的实例是否存在，没有则创建之
        if (!isset(self::$_instance)) {
            self::$_instance = new self();
            self::connect($host, $dbname, $username, $userpass, $charset);
        }
     ...
     */
    protected static function connect($host, $dbname, $username, $userpass, $charset)
    {
        try {
            self::$_link = new PDO("mysql:host={$host};dbname={$dbname};charset=utf8", $username, $userpass);
            //设置返回数据的类型
